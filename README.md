...
- 본인의 프로필 페이지에서는 글 작성 버튼, 내가 작성한 댓글 보기 버튼과 회원 정보를 수정, 로그아웃을 위한 톱니바퀴 버튼이 보인다.
...
- 각 파라미터 값을 Model을 이용하여 글번호, 제목, 이미지, 게시판 카테고리명, 작성자, 작성일, 조회수, USER 번호을 detailView.jsp로 보낸다.
...
1) BoardDAO.java
- detailView.jsp 하단의 댓글창에 댓글을 작성하면 해당 게시물 글번호(num), 로그인하여 댓글을 작성한 USER 아이디(writer), 댓글 내용(content)을 매개변수로 하는 mapper 메소드를 생성한다.
```
